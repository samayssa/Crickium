...
REFERRAL_TIERS: dict[int, dict] = {
    ...
    5: {
        "coins": 200_000,
        "rubies": 250,
        "players": [{"kind": "global", "min": 80, "max": 89}],
    },
    6: {
        "coins": 200_000,
        "rubies": 250,
    ...
    8: {
        "coins": 400_000,
        "rubies": 400,
        "players": [{"kind": "global", "min": 90, "max": 93}],
    },
    9: {
        "coins": 400_000,
        "rubies": 400,
        "players": [{"kind": "special", "min": 90, "max": 98}],
    },
    10: {
        "coins": 700_000,
        "rubies": 650,
        "players": [{"kind": "special", "min": 90, "max": 97}],
    },
    15: {
        "coins": 1_200_000,
        "rubies": 900,
...
def tier_for_referral(number: int) -> dict:
    ...
    earlier = max(
        (n for n in MILESTONE_LEVELS if n < number),
        default=1,
    )
...
def reward_label(number: int) -> str:
    ...
    return (
        f"🪙 {int(tier['coins']):,} Coins • "
        f"💎 {int(tier['rubies']):,} Rubies"
    )


def build_referral_prize_structure() -> str:
    """Return a compact tiered referral prize summary.

    Consecutive referral numbers with the same coin/ruby reward are grouped
    together, while milestone referrals are shown separately when they add
    player-card rewards.
    """
    lines = [
        "1-2. 50,000 Coins + 50 Rubies each.",
        "3-4. 100,000 Coins + 100 Rubies each.",
        (
            "5-7. 200,000 Coins + 250 Rubies + "
            "1 random non-special player (Lv 80-89) each."
        ),
        (
            "8. 400,000 Coins + 400 Rubies + "
            "1 random non-special player (Lv 90-93)."
        ),
        (
            "9. 400,000 Coins + 400 Rubies + "
            "1 random Special Edition player (Lv 90-98)."
        ),
        (
            "10. 700,000 Coins + 650 Rubies + "
            "1 random Special Edition player (Lv 90-97)."
        ),
        "11-14. 700,000 Coins + 650 Rubies each, no player.",
        (
            "15. 1,200,000 Coins + 900 Rubies + "
            "1 non-special player (Lv 90-94) + "
            "1 Special Edition player (Lv 90-98)."
        ),
        "16-19. 1,200,000 Coins + 900 Rubies each, no player.",
        (
            "20. 1,700,000 Coins + 1,150 Rubies + "
            "1 non-special player (Lv 90-94) + "
            "1 Special Edition player (Lv 90-98)."
        ),
        "21-24. 1,700,000 Coins + 1,150 Rubies each, no player.",
        (
            "25. 2,200,000 Coins + 1,400 Rubies + "
            "1 non-special player (Lv 90-94) + "
            "1 Special Edition player (Lv 90-98)."
        ),
        "26-29. 2,200,000 Coins + 1,400 Rubies each, no player.",
        (
            "30. 2,700,000 Coins + 1,650 Rubies + "
            "1 non-special player (Lv 90-94) + "
            "1 Special Edition player (Lv 90-98)."
        ),
        "31-39. 2,700,000 Coins + 1,650 Rubies each, no player.",
        (
            "40. 3,700,000 Coins + 2,150 Rubies + "
            "1 non-special player (Lv 90-94) + "
            "1 Special Edition player (Lv 90-98)."
        ),
        "41-49. 3,700,000 Coins + 2,150 Rubies each, no player.",
        (
            "50. 4,700,000 Coins + 2,650 Rubies + "
            "1 non-special player (Lv 90-94) + "
            "1 Special Edition player (Lv 90-98)."
        ),
    ]
...
def build_referral_message(referral_url: str) -> str:
    return (
        ...
        "<b>Invite friends and earn rewards when they finish "
        "the full Crickium onboarding path.</b>\n\n"
        ...
        "4. They must complete at least one regular game. "
        "Super Over-only play does not count.\n\n"
        "⚠️ The referred user must be new: an existing database user "
        "or an existing debut squad makes the referral ineligible."
        "</blockquote>\n\n"
        "<blockquote expandable>"
        f"<b>🎯 PRIZE STRUCTURE</b>\n"
        f"{html.escape(build_referral_prize_structure())}"
        "</blockquote>\n\n"
        f"<blockquote><b>🔗 YOUR REFERRAL LINK</b>\n"
        f"<code>{html.escape(referral_url)}</code></blockquote>\n\n"
        "<i>The reward is credited automatically to your account after "
        "the referred user completes every required step.</i>\n\n"
        "<b>╰━━━━━━━━━━━━━━━━━━━━╯</b>"
    )
...
async def _qualification_flags(
    conn,
    user_id: int,
) -> tuple[bool, bool, bool]:
    debut = (
        await conn.fetchval(
            "SELECT 1 FROM team_squads WHERE user_id=$1 LIMIT 1;",
            user_id,
        )
        is not None
    )
    ...
    claim = (
        int(
            await conn.fetchval(
                ...
                FROM player_claims
                WHERE user_id=$1
                  AND status IN ('retained','released');
                """,
                user_id,
            )
            or 0
...
async def refresh_referral_progress(user_id: int) -> dict | None:
    ...
    async def _tx(conn):
        row = await conn.fetchrow(
            "SELECT * FROM referrals WHERE referred_id=$1 FOR UPDATE;",
            user_id,
        )
        ...
        debut_done, claim_done, game_done = await _qualification_flags(
            conn,
            user_id,
        )
        ...
        # Lock the referrer wallet row before calculating the next reward
        # number. This serializes concurrent referral completions.
        user_row = await conn.fetchrow(
            """
            SELECT balance, rubies
            FROM users
            WHERE user_id=$1
            FOR UPDATE;
            """,
        ...
        for spec in tier["players"]:
            ...
            if kind == "special":
                p = await conn.fetchrow(
                    ...
                    WHERE GREATEST(
                        COALESCE(bat_level,0),
                        COALESCE(bowl_level,0)
                    ) BETWEEN $1 AND $2
                    ORDER BY random()
                    LIMIT 1;
                    """,
                ...
                [
                    {
                        ...
                        "level": max(
                            int(p.get("bat_level") or 0),
                            int(p.get("bowl_level") or 0),
                        ),
                    }
...
def build_my_referrals_message(rows: list[dict]) -> str:
    ...
    if completed:
        chunks.append(
            "<blockquote expandable><b>✅ COMPLETED</b>"
        )
        ...
        for r in completed:
            ...
            chunks.append(
                ...
                f"<code>{int(r['referred_id'])}</code> → "
                f"Referral #{num} → "
                f"{reward_label(int(num))}"
            )
    ...
    if pending:
        chunks.append(
            "<blockquote expandable><b>⏳ INCOMPLETE</b>"
        )

        for r in pending:
            steps = [
                "✅ Debut"
                if r.get("debut_completed")
                else "⬜ Debut",
                "✅ Claim"
                if r.get("claim_completed")
                else "⬜ Claim",
                "✅ Game"
                if r.get("game_completed")
                else "⬜ Game",
            ]
    ...
    if not rows:
        chunks.append(
            "<blockquote>"
            "You have no referral records yet. "
            "Send /refer to invite your friends."
            "</blockquote>"
        )
...
def share_url(referral_url: str, first_name: str) -> str:
    text = (
        "Join me on Crickium and unlock "
        "the referral rewards! 🎁"
    )

    return (
        "https://t.me/share/url?url="
        + quote_plus(referral_url)
        + "&text="
        + quote_plus(text)
    )