...
def _challenge_text(a,o):
    return ("<b>╭━━〔 🏏 IPL MATCH 〕━━╮\n\n"
            ...
            "<b>💬 IPL challenge received!\n\n"
            "Choose your IPL franchise, build your Playing XI,\n"
...
@register('playipl')
async def playipl_command(message):
    chat_id=int(message['chat']['id']); u=message.get('from',{}); challenger_id=int(u.get('id'))
    active=await get_active_match_in_chat(chat_id)
    if active:
        await app.send_message(chat_id,'<b>⚠️ A PlayIPL game is already going on in this group.</b>',parse_mode='HTML'); return
    ...
    if reply.get('id'):
        ...
        if parsed is None:
            await app.send_message(chat_id,'<b>⚠️ To challenge someone, reply to their message with /playipl, use /playipl @username, or use /playipl USER_ID.</b>',parse_mode='HTML'); return
        kind,val=parsed
...
@register_callback('playipl_accept')
async def playipl_accept(callback_query):
